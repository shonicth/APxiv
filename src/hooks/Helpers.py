...
def before_is_location_enabled(multiworld: MultiWorld, player: int, location: dict[str, Any]) -> bool | None:
    ...
    if "Deep Dungeon" in location["category"][0] and "extra_number" in location:
        if location["region"] == "The Palace of the Dead" and get_int_value(multiworld, player, "include_potd"):
            return Helpers.is_option_enabled(multiworld, player, "deep_dungeon_sanity")
        if location["region"] == "Heaven-on-High"and get_int_value(multiworld, player, "include_hoh"):
            return Helpers.is_option_enabled(multiworld, player, "deep_dungeon_sanity")
        if location["region"] == "Eureka Orthos"and get_int_value(multiworld, player, "include_eo"):
            return Helpers.is_option_enabled(multiworld, player, "deep_dungeon_sanity")
        if location["region"] == "Pilgrim's Traverse"and get_int_value(multiworld, player, "include_pt"):
            return Helpers.is_option_enabled(multiworld, player, "deep_dungeon_sanity")
        return False
